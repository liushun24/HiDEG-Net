...
def save_table_generic(output_dir, filename_prefix, headers, rows, fig_width=13.5, fig_height=2.4, font_size=8.5):
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        ...
        f.write(sep_line + "\n")
        for r in rows:
            f.write("".join(str(r[j]).ljust(col_widths[j]) for j in range(len(headers))) + "\n")
    ...
    savefig_paper(fig, png_path)

    print(f"Saved table: {filename_prefix}")
    print(f"  {csv_path}")
    print(f"  {txt_path}")
    print(f"  {png_path}")


def save_method_comparison_table(
    output_dir,
    dofs,
    num_elements,
    fem_solve_time,
    ...
    rows = [
        ...
        [
            ...
            "—",
            format_sci(relative_displacement_error),
            format_sci(relative_stress_error),
        ],
    ]

    save_table_generic(
        output_dir=output_dir,
        filename_prefix="method_comparison",
        headers=headers,
        rows=rows,
        fig_width=13.5,
        fig_height=2.4,
...
def save_case_summary_table(
    ...
    rows = [[
        ...
        format_sci(e_sigma),
        format_sci(e_Pi),
        format_sci(max_displacement_error),
        format_sci(max_stress_error),
        format_sci(bc_violation),
        format_seconds(training_time),
        format_seconds(inference_time),
        format_seconds(fem_time),
    ]]

    save_table_generic(
        output_dir=output_dir,
        filename_prefix="case_summary",
        headers=headers,
        rows=rows,
        fig_width=17.0,
        fig_height=2.2,
...
def fast_sparse_energy_loss(
    ...
    This version intentionally removes the residual-equilibrium term and trains only by
    minimizing the total potential energy:
        Pi = 1/2 u^T K u - u^T f
    """
...
def evaluate_errors(u_pred, u_fem, elem_stress_pred, elem_stress_fem, vm_pred, vm_fem):
    ...
    return {
        "displacement_rel_l2_all": relative_l2_error(u_pred.reshape(-1), u_fem.reshape(-1)),
        "stress_rel_l2_all": relative_l2_error(elem_stress_pred.reshape(-1), elem_stress_fem.reshape(-1)),
...
def compute_additional_summary_metrics(
    u_pred_phys,
    u_fem,
    elem_stress_pred,
    elem_stress_fem,
    ...
    disp_node_error = torch.norm(u_pred_phys - u_fem, dim=1)
    max_displacement_error = float(disp_node_error.max().detach().cpu())

    stress_abs_error = torch.abs(elem_stress_pred - elem_stress_fem)
    max_stress_error = float(stress_abs_error.max().detach().cpu())

    Pi_model = energy_model["Pi"]
    Pi_fem = energy_fem["Pi"]
    e_Pi = abs(Pi_model - Pi_fem) / (abs(Pi_fem) + eps)

    bc_left = torch.abs(u_pred_phys[left_nodes, 0]).max() if left_nodes.numel() > 0 else torch.tensor(0.0, device=u_pred_phys.device)
    bc_bottom = torch.abs(u_pred_phys[bottom_nodes, 1]).max() if bottom_nodes.numel() > 0 else torch.tensor(0.0, device=u_pred_phys.device)
    bc_violation = float(torch.maximum(bc_left, bc_bottom).detach().cpu())
...
def save_training_curves(output_dir, history):
    ...
    epochs = history.get("epoch", list(range(1, len(history["loss"]) + 1)))

    # Save the full-resolution training history as CSV.
    history_csv_path = os.path.join(output_dir, "training_history_full.csv")
    with open(history_csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["epoch", "loss", "energy", "U_int", "W_ext", "lr"])
        for i, epoch in enumerate(epochs):
            writer.writerow([
                ...
                history["U_int"][i],
                history["W_ext"][i],
                history["lr"][i] if "lr" in history and i < len(history["lr"]) else "",
            ])

    # Energy-related curves: one point is saved for every training epoch.
    fig, ax = plt.subplots(figsize=(7.0, 4.2))

    ax.plot(epochs, history["loss"], label="loss", linewidth=1.4)
    ax.plot(epochs, history["U_int"], label=r"$U_{\mathrm{int}}$", linewidth=1.2)
    ax.plot(epochs, history["W_ext"], label=r"$W_{\mathrm{ext}}$", linewidth=1.2)
    ...
    # Learning-rate curve over the full training process.
    if "lr" in history and len(history["lr"]) == len(epochs):
        fig, ax = plt.subplots(figsize=(7.0, 4.2))
        ax.plot(epochs, history["lr"], label="learning rate", linewidth=1.3)
        ax.set_xlabel("Epoch")
        ...
        style_axis_paper(ax, equal=False)
        fig.tight_layout()
        savefig_paper(fig, os.path.join(output_dir, "learning_rate_history.png"))

    # Optional error curves are recorded at evaluation epochs only.
    if "eval_epoch" in history and len(history["eval_epoch"]) > 0:
        fig, ax = plt.subplots(figsize=(7.0, 4.2))
        ax.plot(history["eval_epoch"], history["ux_rel_l2"], label=r"$u_x$ rel. $L^2$", linewidth=1.2)
        ax.plot(history["eval_epoch"], history["uy_rel_l2"], label=r"$u_y$ rel. $L^2$", linewidth=1.2)
        ax.plot(history["eval_epoch"], history["umag_rel_l2"], label=r"$|u|$ rel. $L^2$", linewidth=1.2)
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Relative error")
        ax.set_title("Evaluation error history")
        ax.legend(frameon=False)
        style_axis_paper(ax, equal=False)
        fig.tight_layout()
        savefig_paper(fig, os.path.join(output_dir, "evaluation_error_history.png"))
...
# ============================================================
def main():
    output_dir = "quarter_plate_with_hole_sparse_energy_only_full_history"
    case_name = "quarter_plate_with_hole_sparse_energy_only_full_history"
    ensure_dir(output_dir)

    msh_path = os.path.join(output_dir, "plate_with_hole.msh")
    vtu_path = os.path.join(output_dir, "results_quarter_plate_with_hole_sparse_energy_only_full_history.vtu")
    ...
    # -------------------------
    sync_cuda_if_needed()
    fem_solve_t0 = time.perf_counter()
    ...
    sync_cuda_if_needed()
    fem_solve_time = time.perf_counter() - fem_solve_t0
    ...
    history = {
        # Full-resolution training curves: one entry per epoch.
        "epoch": [],
        "loss": [],
        "energy": [],
        "U_int": [],
        ...
        # Evaluation metrics: recorded only at eval_every / print epochs.
        "eval_epoch": [],
        "ux_rel_l2": [],
        "uy_rel_l2": [],
        "umag_rel_l2": [],
    ...
    def evaluate_current_model():
        ...
        with torch.no_grad():
            ...
            loss_eval, logs_eval = fast_sparse_energy_loss(
    ...
    # Training
    # -------------------------
    print("\n================= Training: sparse K @ u, energy-only =================")
    ...
    for epoch in range(1, adamw_epochs + 1):
        ...
        loss, logs = fast_sparse_energy_loss(
            u_pred_tilde=u_pred_tilde,
            K_rows=K_tilde_rows,
            K_cols=K_tilde_cols,
            K_vals=K_tilde_vals,
        ...
        # Record the training curves at every epoch, not only at eval_every.
        # These values are computed during the training step before the parameter update.
        history["epoch"].append(epoch)
        history["loss"].append(logs["loss"])
        history["energy"].append(logs["energy"])
        history["U_int"].append(logs["U_int"])
        history["W_ext"].append(logs["W_ext"])
    ...
    sync_cuda_if_needed()
    training_time = time.perf_counter() - training_t0

    print("================= Training finished =================")
    print(f"Training time = {training_time:.6f} s\n")

    # -------------------------
    # Final prediction / inference timing
    # -------------------------
    model.eval()
    ...
    # -------------------------
    dofs = nodes.shape[0] * 2
    num_elements = elements.shape[0]

    save_method_comparison_table(
        output_dir=output_dir,
        dofs=dofs,
        ...
        relative_displacement_error=metrics["displacement_rel_l2_all"],
        relative_stress_error=metrics["stress_rel_l2_all"],
    )

    # -------------------------
    # Table 2: case summary
    # -------------------------
    summary_metrics = compute_additional_summary_metrics(
        u_pred_phys=u_pred_phys,
        u_fem=u_fem,
        elem_stress_pred=elem_stress_pred,
        elem_stress_fem=elem_stress_fem,
        energy_model=energy_model,
        energy_fem=energy_torch_fem,
        left_nodes=left_nodes,
        bottom_nodes=bottom_nodes,
    )

    save_case_summary_table(
        output_dir=output_dir,
        case_name=case_name,
        e_u=metrics["displacement_rel_l2_all"],
        e_sigma=metrics["stress_rel_l2_all"],
        e_Pi=summary_metrics["e_Pi"],
        max_displacement_error=summary_metrics["max_displacement_error"],
        max_stress_error=summary_metrics["max_stress_error"],
        bc_violation=summary_metrics["bc_violation"],
        training_time=training_time,
        inference_time=inference_time,
        fem_time=fem_solve_time,
    )
    ...
    # Save checkpoint
    # -------------------------
    save_path = os.path.join(output_dir, "gpt_elastic_quarter_plate_sparse_energy_only_full_history.pt")
    ...
    torch.save(
        {
            ...
            "lc_min": lc_min,
            "lc_max": lc_max,
            "note": "Training uses energy-only PyTorch sparse K@u. Residual-equilibrium loss is removed. FEM reference uses SciPy sparse spsolve. No K.to_dense() is used. Training curves are recorded at every epoch.",
        },
        save_path,
    )
